...
def aggregate_hierarchical_model_performance_cst(nClassifiers,
    ...
    :param return_topK: <bool> set to True if the outputted accuracy curve should be of the top-5
                    accuracy instead of the top-1.  In this version, he confidence thresholds
                    are compared against are not the sums of top-5 confidences, but the 
                    ordinary maximal softmaxes.
    ...
    for modelname in modelnames_list:
        with open(os.path.join("Logs", "log_" + modelname + ".txt"), "r") as f:
    ...
    if reReadResult:
        ...
        Probabilities_dict = {}
        Topclasses_dict = {}
        path_to_label_file = os.path.join(dataset_dir, dataset_name, labels_filename)
        ...
        for modelname, confidence_threshold in zip(modelnames_list, perCLFthreshold):
            ...
            Model_dict["n_confident_images"] = int(nConfident_top1)
    ...
    with open(os.path.join(output_dir, "Stats.txt"), "w") as f:
        ...
        f.write("For epsilon={:3.2f} accuracy-top5 is {:4.2f}% accuracy-top1 is {:4.2f}% is speedup is {:6.3f}\n".format(accuracy_compromise,
